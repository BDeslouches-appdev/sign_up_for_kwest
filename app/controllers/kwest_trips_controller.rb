class KwestTripsController < ApplicationController
  def index
    ...
    @kwest_trips = @q.result(:distinct => true).includes(:sign_ups, :recommendations).page(params[:page]).per(10)
  ...
  def create
    ...
    @kwest_trip.region = params[:region]
    @kwest_trip.is_short = params[:is_short]
